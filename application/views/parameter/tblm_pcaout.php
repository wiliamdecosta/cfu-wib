...
<script>
    ...
    function clearInputCorrActivity() {
        $('#form_corractivityid_fk').val('');
        $('#form_corractivitycode').val('');
        $('#form_corractivityname').val('');
    }
    

    function clearInputCorrPLItem() {
        $('#form_corrplitemcode').val('');
        $('#form_corrplitemname').val('');
    }
    ...
    function showData(){
        ...
        jQuery(function($) {
            ...
            jQuery("#grid-table").jqGrid('setGridParam',{
                url: '<?php echo WS_JQGRID."parameter.tblm_pcaout_controller/read"; ?>',
                postData: {
...
<script>
    ...
    jQuery(function($) {
        ...
        jQuery("#grid-table").jqGrid({
            url: '<?php echo WS_JQGRID."parameter.tblm_pcaout_controller/crud"; ?>',
            datatype: "json",
            mtype: "POST",
            colModel: [
                {label: 'ID', name: 'pcaoutid_pk', key: true, width: 5, sorttype: 'number', editable: true, hidden: true},
                ...
                {label: 'Data Source', name: 'casetype_display', width: 100, editable: false, hidden: false},
                {label: 'Data Source',name: 'casetype', width: 200, sortable: true, editable: true, hidden: true,
                    editrules: {edithidden: true, required: true},
                    edittype: 'select',
                    editoptions: {
                        style: "width: 150px", 
                        dataUrl: '<?php echo WS_JQGRID."parameter.tblm_pcaout_controller/combo"; ?>',
                        // "custom_element":function( value  , options) {
                        //     console.log(value);
                        // }
                    }
                ...
                {label: 'Correct Activity Name', name: 'corractivityname', width: 350, editable: false, hidden: true},
                {label: 'Correct Activity', name: 'corractivity_display', width: 350, editable: false, hidden: false},
                {label: 'Correct Activity',
                    ...
                    editoptions: {
                        "custom_element":function( value  , options) {
                            ...
                            // give the editor time to initialize
                            setTimeout( function() {
                                elm.append('<input id="form_corractivityid_fk" type="text"  style="display:none;">'+
                                        '<input id="form_corractivitycode" readonly type="text" class="FormElement form-control" placeholder="Choose Correct Activity">'+
                                        '<button class="btn btn-success" type="button" onclick="showLovActivity(\'form_corractivityid_fk\',\'form_corractivitycode\',\'form_corractivityname\')">'+
                                        '   <span class="fa fa-search bigger-110"></span>'+
                                        '</button> &nbsp;' +
                                        '<input id="form_corractivityname" readonly type="text" size="30" class="FormElement form-control" placeholder="Correct  Activity Name">');
                                $("#form_corractivitycode").val(value);
                                elm.parent().removeClass('jqgrid-required');
                            }, 100);
                        ...
                        "custom_value":function( element, oper, gridval) {
                            ...
                            if(oper === 'get') {
                                return $("#form_corractivitycode").val();
                            } else if( oper === 'set') {
                                $("#form_corractivitycode").val(gridval);
                                var gridId = this.id;
                                // give the editor time to set display
                                setTimeout(function(){
                                    var selectedRowId = $("#"+gridId).jqGrid ('getGridParam', 'selrow');
                                    if(selectedRowId != null) {
                                        var code_display = $("#"+gridId).jqGrid('getCell', selectedRowId, 'correctactivity');
                                        var name_display = $("#"+gridId).jqGrid('getCell', selectedRowId, 'corractivityname');
                                        $("#form_corractivitycode").val( code_display );
                                        $("#form_corractivityname").val( name_display );
                                    }
                                },100);
                            }
                        }
                ...
                {label: 'Correct PL Item Name', name: 'corrplitemname', width: 300, editable: false, hidden: false},
                {label: 'Correct PL Item', name: 'correctplitem_display', width: 300, editable: false, hidden: false},
                {label: 'Correct PL Item',
                    name: 'correctplitem',
                    width: 200,
                    sortable: true,
                    editable: true,
                    ...
                    editoptions: {
                        "custom_element":function( value  , options) {
                            ...
                            // give the editor time to initialize
                            setTimeout( function() {
                                elm.append('<input id="form_corrplitemcode" type="text" readonly class="FormElement form-control" placeholder="Choose Correct PL Item">'+
                                        '<button class="btn btn-success" type="button" onclick="showLOVPLItem(\'form_corrplitemcode\',\'form_corrplitemname\')">'+
                                        '   <span class="fa fa-search bigger-110"></span>'+
                                        '</button> &nbsp;' +
                                        '<input id="form_corrplitemname" readonly type="text" size="30" class="FormElement form-control" placeholder="Correct PL Item Name">');
                                $("#form_corrplitemcode").val(value);
                                elm.parent().removeClass('jqgrid-required');
                            }, 100);
                        ...
                        "custom_value":function( element, oper, gridval) {
                            ...
                            if(oper === 'get') {
                                return $("#form_corrplitemcode").val();
                            } else if( oper === 'set') {
                                $("#form_corrplitemcode").val(gridval);
                                var gridId = this.id;
                                // give the editor time to set display
                                setTimeout(function(){
                                    var selectedRowId = $("#"+gridId).jqGrid ('getGridParam', 'selrow');
                                    if(selectedRowId != null) {
                                        var code_display = $("#"+gridId).jqGrid('getCell', selectedRowId, 'corrplitemname');
                                        $("#form_corrplitemname").val( code_display );
                                    }
                                },100);
                            }
                        }
            ...
            editurl: '<?php echo WS_JQGRID."parameter.tblm_pcaout_controller/crud"; ?>',
            caption: "PCA Taken Out"
            ...
            },
            {
                ...
                beforeShowForm: function (e, form) {
                    ...
                    setTimeout(function() {
                        ...
                        clearInputActivity();
                        clearGLAccount();
                        clearInputCorrActivity();
                        clearInputCorrPLItem();
                    },100);
