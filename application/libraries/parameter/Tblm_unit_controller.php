...
class Tblm_unit_controller {
    ...
    function read() {
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('parameter/tblm_unit');
            $table = $ci->tblm_unit;
            ...
            if(!empty($i_search)) {
                $table->setCriteria("(upper(code) like upper('%".$i_search."%') OR
                                         upper(unitname) like upper('%".$i_search."%')
                                         )");
